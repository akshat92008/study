...
describe('MIND Context & Prompts', () => {
  ...
  it('frames the AI Tutor around the unified learner state loop', () => {
    const prompt = getMINDSystemPrompt({
      ...
      currentSessionCard: {
        ...
        subject: 'Chemistry',
        estimatedMinutes: 30,
        rationale: 'Recent Mistake Review signals and due Review cards point here.',
      },
    ...
    expect(prompt).toContain('You are the Cognition OS AI Tutor');
    expect(prompt).toContain('Selected learning goal: NEET Biology sprint');
    ...
    expect(prompt).toContain('If Review has due cards');
    expect(prompt).toContain('guide them to Mistake Review');
