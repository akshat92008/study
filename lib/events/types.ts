...
export const EventPayloadSchemas: Partial<Record<EventType | string, z.ZodTypeAny>> = {
  ...
  AUTOPSY_V3_ASSESSMENT_CREATED: z.object({
    assessmentId: z.string().uuid(),
    goalId: MaybeUuid,
  }).passthrough(),
  AUTOPSY_V3_QUESTIONS_UPSERTED: z.object({
    assessmentId: z.string().uuid(),
    questionCount: z.number().int().nonnegative().optional(),
  }).passthrough(),
  AUTOPSY_V3_REASONS_COLLECTED: z.object({
    assessmentId: z.string().uuid(),
    diagnosisCount: z.number().int().nonnegative().optional(),
  }).passthrough(),
  AUTOPSY_V3_REPORT_READY: z.object({
    assessmentId: z.string().uuid(),
