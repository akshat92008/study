...
export async function generateKnowledgeUpdate(
  ...
  for (const question of diagnosedIncorrect) {
    ...
    const { error } = await supabase.from('mistakes').insert({
      ...
      mistake_text: question.questionText ?? question.reasoning ?? `Mistake in ${topic || chapter}`,
      ...
      why_wrong: question.reasoning ?? null,
      exam_trap: question.correctExplanation ?? question.conceptualGap ?? null,
      severity: Math.max(1, Math.min(5, Math.ceil(Number(question.marksLost ?? 1)))),
