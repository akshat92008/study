...
```
Build the main Command Center — the /dashboard home page.
This is the first thing the student sees. It should feel like an F1 telemetry dashboard.
Shows: daily overview, quick stats, AI insight, today's tasks, due revisions, recent mistakes.
Pulls data from all engines via server actions. Style with CSS variables. NO Tailwind.
```
...
## STEP 2: Command Center Component — `components/dashboard/CommandCenter.tsx`
...
```bash
npm run dev
# Visit /dashboard — full command center with all data aggregated
# Cards link to respective pages
# Today's tasks appear from planner
# Quick actions provide entry points to all features
```

**→ NEXT: MODULE 14 (Onboarding) & MODULE 15 (Landing Page)**
