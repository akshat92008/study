...
describe('chat AI budget lifecycle contract', () => {
  it('keeps /api/ai/chat on the atomic cost guard and off deprecated usage tracking', () => {
    ...
    expect(chatRoute).toContain('budgetedGenerateJSON');
    expect(chatRoute).toContain('budgetedStreamGeneration');
    expect(chatRoute).toContain('budgetedVisionCall');
    ...
    expect(chatRoute).not.toContain('reserveBudgetForModelCall');
    expect(chatRoute).not.toContain('releaseBudgetReservation');
