    ...
    <div className="animate-fade" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-6)' }}>
      ...
      <div className="grid-3 stagger">
        <Card variant="glow">
          ...
          <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-black)', fontFamily: 'var(--font-mono)', color: 'var(--danger)' }}>-{totalMarksLost}</div>
        </Card>
        <Card>
          <div className="label">Mistakes Logged</div>
          <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-black)', fontFamily: 'var(--font-mono)', color: 'var(--warning)' }}>{mistakes.length}</div>
        </Card>
        <Card>
          <div className="label">Top Issue</div>
          <div style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-bold)', marginTop: 'var(--sp-1)' }}>{patterns[0]?.category?.replace('_', ' ') || 'None yet'}</div>
        </Card>
      </div>
        ...
        <Card padding="lg" className="animate-fade">
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-4)' }}>
            ...
            <div>
              ...
              <select name="category" style={{ width: '100%', padding: 'var(--sp-2) var(--sp-3)', background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)' }}>
                {CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)', display: 'block', marginBottom: 'var(--sp-1)' }}>Marks Lost</label>
              <input name="marksLost" type="number" defaultValue={4} style={{ width: '100%', padding: 'var(--sp-2) var(--sp-3)', background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)' }} />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)', display: 'block', marginBottom: 'var(--sp-1)' }}>Question (optional)</label>
              <textarea name="questionText" rows={2} style={{ width: '100%', padding: 'var(--sp-2) var(--sp-3)', background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', resize: 'vertical' }} />
            </div>
      ...
      {patterns.length > 0 && (
        <Card>
          <h3 style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-semibold)', marginBottom: 'var(--sp-4)' }}><AlertTriangle size={18} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 'var(--sp-2)', color: 'var(--warning)' }} />Mistake Patterns</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)' }}>
            {patterns.map((p: any) => {
              const cat = CATEGORIES.find(c => c.value === p.category);
              return (
                <div key={p.category} style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', padding: 'var(--sp-3)', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                  <Badge color={cat?.color || 'gray'}>{p.category.replace('_', ' ')}</Badge>
                  <div style={{ flex: 1 }}><div style={{ height: 6, borderRadius: 3, background: 'var(--bg-hover)', overflow: 'hidden' }}><div style={{ height: '100%', width: `${p.percentage}%`, background: 'var(--danger)' }} /></div></div>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{p.count}x</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-sm)', color: 'var(--danger)' }}>-{p.marksLost}</span>
                </div>
              );
            })}
          </div>
      ...
      {mistakes.length > 0 && (
        <Card>
          <h3 style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-semibold)', marginBottom: 'var(--sp-4)' }}>Recent Mistakes</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
            {mistakes.slice(0, 10).map((m: any) => (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', padding: 'var(--sp-3)', borderBottom: '1px solid var(--border-subtle)' }}>
                <Badge color={CATEGORIES.find(c => c.value === m.category)?.color || 'gray'}>{m.category.replace('_', ' ')}</Badge>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-medium)' }}>{m.subject} — {m.chapter}</div>
                  {m.ai_analysis && <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 2 }}>{m.ai_analysis}</div>}
                </div>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-sm)', color: 'var(--danger)' }}>-{m.marks_lost}</span>
              </div>
            ))}
          </div>
        </Card>
