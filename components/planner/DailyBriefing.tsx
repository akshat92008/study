...
const TONE_COLORS: Record<string, string> = {
  push: 'var(--accent-cyan)', encourage: 'var(--success)', calm: 'var(--warning)', rest: 'var(--danger)',
};
...
export default function DailyBriefing() {
  ...
  const progressPct = briefing.progress.total > 0 ? Math.round((briefing.progress.completed / briefing.progress.total) * 100) : 0;
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontSize: 'var(--fs-xl)', fontWeight: 'var(--fw-bold)' }}>Mission Briefing</h2>
        ...
        <div style={{ display: 'flex', gap: 'var(--sp-4)', alignItems: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-black)', color: 'var(--warning)' }}>{briefing.daysRemaining}</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: 'var(--ls-wide)' }}>days left</div>
          </div>
          {briefing.streak > 0 && (
      ...
      {briefing.mood.state === 'overwhelmed' && (
        <Card padding="md" style={{ background: 'var(--danger-dim)', border: '1px solid var(--danger)', display: 'flex', gap: 'var(--sp-3)', alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginTop: 2 }}>Cognitive overload detected. Daily targets reduced by 60%. Prioritizing retention and rest.</div>
          </div>
        </Card>
      )}
