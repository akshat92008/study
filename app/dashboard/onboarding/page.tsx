...
import DynamicCurriculumGenerator from '@/components/onboarding/DynamicCurriculumGenerator';

export default function OnboardingPage() {
  const [step, setStep] = useState(0);
  const [mode, setMode] = useState<'standard' | 'dynamic' | null>(null);
  ...
  const steps = [
    ...
    <div key={0} style={{ textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: 'var(--sp-6)', maxWidth: 400, margin: '0 auto var(--sp-6)' }}>
        Let's set up your AI academic operating system. Choose your path below.
      </p>
      
      <div style={{ display: 'flex', gap: 'var(--sp-4)', flexDirection: 'column' }}>
        <Button onClick={() => { setMode('standard'); setStep(1); }} size="lg" variant="secondary" style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span>Standard Curriculum (NEET, JEE, SAT, etc.)</span>
          <ArrowRight size={18} />
        </Button>
        <Button onClick={() => { setMode('dynamic'); setStep(1); }} size="lg" style={{ display: 'flex', justifyContent: 'space-between', background: 'var(--accent-purple)' }}>
          <span>Build Custom Curriculum (AI Generated)</span>
          <Brain size={18} />
        </Button>
      </div>
    </div>,
    ...
    <div key={1}>
      {mode === 'standard' ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-4)' }}>
          <h2 style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-bold)' }}>Your Standard Goal</h2>
          <div>
  ...
  return (
    <div style={{
      ...
      <Card padding="lg" style={{ maxWidth: 540, width: '100%' }} className="animate-fade">
        {steps[step]}
      </Card>
    </div>
  );
